...
func TestRouterDict(T *testing.T) {
	...
	res = CreateRequest("DELETE", "http://localhost:6505/dict?WORD=define")
	assert.Equal("200 OK", res.Status, "Delete Success")
	res.Body.Close()

	res = CreateRequest("DELETE", "http://localhost:6505/dict?WORD=define")
	assert.Equal("400 Bad Request", res.Status, "you can't delete object was already deleted")
	res.Body.Close()
}
